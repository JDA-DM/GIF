...
- [2. 概要](#2-概要)
  ...
  - [2.2. 目的](#22-目的)
  - [2.3. 概要](#23-概要)
    - [2.3.1. メリット・デメリット](#231-メリットデメリット)
    - [2.3.2. 留意事項](#232-留意事項)
  - [2.4. 　原則](#24-原則)
    - [2.4.1. 参照データモデルとしての整備](#241-参照データモデルとしての整備)
    - [2.4.2. 既存データモデルの活用](#242-既存データモデルの活用)
    - [2.4.3. グローバル標準との整合性](#243-グローバル標準との整合性)
    - [2.4.4. スモールスタートと成果の可視化](#244-スモールスタートと成果の可視化)
    - [2.4.5. イノベーションのための成長の仕組み](#245-イノベーションのための成長の仕組み)
    - [2.4.6. 多様な用途に永く使える仕組み](#246-多様な用途に永く使える仕組み)
    - [2.4.7. データ定義の順次整備](#247-データ定義の順次整備)
  - [2.5. 　アーキテクチャやAPIとの関係](#25-アーキテクチャやapiとの関係)
    - [2.5.1. 動的・静的データ](#251-動的静的データ)
    - [2.5.2. 地理空間データ](#252-地理空間データ)
    - [2.5.3. パーソナルデータ](#253-パーソナルデータ)
  - [2.6. 　対象範囲](#26-対象範囲)
- [3. 　本書について](#3-本書について)
  - [3.1. 　本書の全体構成](#31-本書の全体構成)
    - [3.1.1. コアデータパーツ](#311-コアデータパーツ)
    - [3.1.2. データモデル](#312-データモデル)
    - [3.1.3. コード体系](#313-コード体系)
    - [3.1.4. カタログ](#314-カタログ)
    - [3.1.5. データの運用](#315-データの運用)
  - [3.2. 　本書の使い方](#32-本書の使い方)
    - [3.2.1. データを設計する場合](#321-データを設計する場合)
    - [3.2.2. データ連携する場合](#322-データ連携する場合)
  - [3.3. 　関連取り組みとの関連性](#33-関連取り組みとの関連性)
    - [3.3.1. 政府相互運用性フレームワークとの関係性](#331-政府相互運用性フレームワークとの関係性)
    - [3.3.2. 地理空間情報、CityGMLとの関係性](#332-地理空間情報citygmlとの関係性)
    - [3.3.3. Smart Data Modelとの関係性](#333-smart-data-modelとの関係性)
    - [3.3.4. Schema.orgとの関係性](#334-schemaorgとの関係性)
    - [3.3.5. デジタル・ツインとの関係性](#335-デジタルツインとの関係性)
    - [3.3.6. 推奨データセットとの関係性](#336-推奨データセットとの関係性)
- [4. 　すべてのデータモデルへの共通事項](#4-すべてのデータモデルへの共通事項)
  - [4.1. 　文字](#41-文字)
  - [4.2. 　外国語表記・ピクトグラム](#42-外国語表記ピクトグラム)
  - [4.3. 　日時](#43-日時)
    - [4.3.1. 定期スケジュール](#431-定期スケジュール)
      - [4.3.1.1. 年次スケジュール](#4311-年次スケジュール)
      - [4.3.1.2. 月次スケジュール](#4312-月次スケジュール)
      - [4.3.1.3. 週次スケジュール](#4313-週次スケジュール)
    - [4.3.2. 非定期のスケジュール](#432-非定期のスケジュール)
  - [4.4. 　緯度、経度](#44-緯度経度)
  - [4.5. 　住所等（アドレス）](#45-住所等アドレス)
    - [4.5.1. 都道府県分離表記方式（町字ID想定）](#451-都道府県分離表記方式町字id想定)
    - [4.5.2. 都道府県分離表記方式（町字ID非想定）](#452-都道府県分離表記方式町字id非想定)
    - [4.5.3. 全住所一体方式](#453-全住所一体方式)
  - [4.6. 　連絡先](#46-連絡先)
  - [4.7. 　画像](#47-画像)
  - [4.8. 　空間スキーマ](#48-空間スキーマ)
  - [4.9. 　レコード情報](#49-レコード情報)
  - [4.10. 　アクセシビリティデータ](#410-アクセシビリティデータ)
  - [4.11. 　子育て支援情報データ](#411-子育て支援情報データ)
  - [4.12. 　言語対応データ](#412-言語対応データ)
  - [4.13. 　申込・予約](#413-申込予約)
  - [4.14. 　支払](#414-支払)
- [5. 　実装データモデル](#5-実装データモデル)
  - [5.1. 　土地](#51-土地)
    - [5.1.1. 土地](#511-土地)
      - [5.1.1.1. 参照元データ](#5111-参照元データ)
      - [5.1.1.2. データモデル](#5112-データモデル)
      - [5.1.1.3. データ項目に選択肢があるときの候補](#5113-データ項目に選択肢があるときの候補)
        - [5.1.1.3.1. 土地の用途](#51131-土地の用途)
      - [5.1.1.4. 既存のデータモデル](#5114-既存のデータモデル)
        - [5.1.1.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#51141-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.1.1.4.2. 不動産登記情報](#51142-不動産登記情報)
        - [5.1.1.4.3. 電子国土基本図　地図情報ファイル仕様書](#51143-電子国土基本図地図情報ファイル仕様書)
        - [5.1.1.4.4. 国土数値情報](#51144-国土数値情報)
        - [5.1.1.4.5. 政府関連土地関連基礎情報](#51145-政府関連土地関連基礎情報)
        - [5.1.1.4.6. Smart Data Model](#51146-smart-data-model)
        - [5.1.1.4.7. Schema.org](#51147-schemaorg)
    - [5.1.2. 都市計画基礎調査情報](#512-都市計画基礎調査情報)
  - [5.2. 　建物](#52-建物)
      - [5.2.0.1. 参照元データ](#5201-参照元データ)
      - [5.2.0.2. データモデル](#5202-データモデル)
      - [5.2.0.3. データ項目に選択肢があるときの候補](#5203-データ項目に選択肢があるときの候補)
        - [5.2.0.3.1. 種別](#52031-種別)
        - [5.2.0.3.2. 構造](#52032-構造)
        - [5.2.0.3.3. 主要用途](#52033-主要用途)
      - [5.2.0.4. 既存データモデル](#5204-既存データモデル)
        - [5.2.0.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#52041-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.2.0.4.2. 不動産登記情報](#52042-不動産登記情報)
        - [5.2.0.4.3. 電子国土基本図　地図情報ファイル仕様書](#52043-電子国土基本図地図情報ファイル仕様書)
        - [5.2.0.4.4. Smart Data Model](#52044-smart-data-model)
        - [5.2.0.4.5. Schema.org](#52045-schemaorg)
  - [5.3. 　施設](#53-施設)
    - [5.3.1. 基本形](#531-基本形)
      - [5.3.1.1. 参照元データ](#5311-参照元データ)
      - [5.3.1.2. データモデル](#5312-データモデル)
      - [5.3.1.3. データ項目に選択肢があるときの候補](#5313-データ項目に選択肢があるときの候補)
        - [5.3.1.3.1. POIコード](#53131-poiコード)
      - [5.3.1.4. 既存データモデル](#5314-既存データモデル)
        - [5.3.1.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53141-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.1.4.2. 電子国土基本図　地図情報ファイル仕様書](#53142-電子国土基本図地図情報ファイル仕様書)
        - [5.3.1.4.3. 国土数値情報](#53143-国土数値情報)
        - [5.3.1.4.4. Smart Data Model](#53144-smart-data-model)
        - [5.3.1.4.5. Schema.org](#53145-schemaorg)
    - [5.3.2. 公共施設・観光施設](#532-公共施設観光施設)
      - [5.3.2.1. 参照元データ](#5321-参照元データ)
      - [5.3.2.2. データモデル](#5322-データモデル)
      - [5.3.2.3. データ項目に選択肢があるときの候補](#5323-データ項目に選択肢があるときの候補)
      - [5.3.2.4. 既存データモデル](#5324-既存データモデル)
        - [5.3.2.4.1. 国土数値情報](#53241-国土数値情報)
        - [5.3.2.4.2. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53242-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.2.4.3. Smart Data Model](#53243-smart-data-model)
        - [5.3.2.4.4. Schema.org](#53244-schemaorg)
    - [5.3.3. 医療機関](#533-医療機関)
      - [5.3.3.1. 参照元データ](#5331-参照元データ)
      - [5.3.3.2. データモデル](#5332-データモデル)
      - [5.3.3.3. データ項目に選択肢があるときの候補](#5333-データ項目に選択肢があるときの候補)
        - [5.3.3.3.1. 医療機関区分](#53331-医療機関区分)
        - [5.3.3.3.2. 診療科目](#53332-診療科目)
        - [5.3.3.3.3. 医療機関番号](#53333-医療機関番号)
        - [5.3.3.3.4. 災害拠点分類](#53334-災害拠点分類)
      - [5.3.3.4. 既存データモデル](#5334-既存データモデル)
        - [5.3.3.4.1. 厚生労働省各地方局の「保険医療機関・保険薬局の指定等一覧」](#53341-厚生労働省各地方局の保険医療機関保険薬局の指定等一覧)
        - [5.3.3.4.2. 国土数値情報の「医療機関」](#53342-国土数値情報の医療機関)
        - [5.3.3.4.3. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53343-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.3.4.4. Schema.org](#53344-schemaorg)
    - [5.3.4. 教育機関](#534-教育機関)
      - [5.3.4.1. 参照元データ](#5341-参照元データ)
      - [5.3.4.2. データモデル](#5342-データモデル)
      - [5.3.4.3. 既存のデータモデル](#5343-既存のデータモデル)
        - [5.3.4.3.1. 文部科学省　学校コード 教育データ標準](#53431-文部科学省学校コード-教育データ標準)
        - [5.3.4.3.2. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53432-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.4.3.3. Schema.org](#53433-schemaorg)
    - [5.3.5. 介護サービス事業所](#535-介護サービス事業所)
      - [5.3.5.1. 参照元データ](#5351-参照元データ)
      - [5.3.5.2. データモデル](#5352-データモデル)
      - [5.3.5.3. データ項目に選択肢があるときの候補](#5353-データ項目に選択肢があるときの候補)
        - [5.3.5.3.1. 実施サービス](#53531-実施サービス)
      - [5.3.5.4. 既存のデータモデル](#5354-既存のデータモデル)
        - [5.3.5.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53541-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.5.4.2. Schema.org](#53542-schemaorg)
    - [5.3.6. 子育て支援施設](#536-子育て支援施設)
      - [5.3.6.1. 参照元データ](#5361-参照元データ)
      - [5.3.6.2. データモデル](#5362-データモデル)
      - [5.3.6.3. データ項目に選択肢があるときの候補](#5363-データ項目に選択肢があるときの候補)
        - [5.3.6.3.1. POIコード](#53631-poiコード)
    - [5.3.7. 事業所](#537-事業所)
      - [5.3.7.1. 参照元データ](#5371-参照元データ)
      - [5.3.7.2. データモデル](#5372-データモデル)
      - [5.3.7.3. データ項目に選択肢があるときの候補](#5373-データ項目に選択肢があるときの候補)
        - [5.3.7.3.1. 業種](#53731-業種)
      - [5.3.7.4. 既存のデータモデル](#5374-既存のデータモデル)
        - [5.3.7.4.1. Smart Data Model](#53741-smart-data-model)
        - [5.3.7.4.2. Schema.org](#53742-schemaorg)
    - [5.3.8. 食品等営業許可・届出事業所](#538-食品等営業許可届出事業所)
      - [5.3.8.1. 参照元データ](#5381-参照元データ)
      - [5.3.8.2. データモデル](#5382-データモデル)
      - [5.3.8.3. データ項目に選択肢があるときの候補](#5383-データ項目に選択肢があるときの候補)
        - [5.3.8.3.1. 食品衛生法施行令第35条](#53831-食品衛生法施行令第35条)
    - [5.3.9. 公園](#539-公園)
      - [5.3.9.1. 参照元データ](#5391-参照元データ)
      - [5.3.9.2. データモデル](#5392-データモデル)
      - [5.3.9.3. データ項目内で選択肢があるときの候補](#5393-データ項目内で選択肢があるときの候補)
        - [5.3.9.3.1. 国土数値情報](#53931-国土数値情報)
      - [5.3.9.4. 既存データモデル](#5394-既存データモデル)
        - [5.3.9.4.1. 国土数値情報](#53941-国土数値情報)
        - [5.3.9.4.2. ３Ｄ都市モデル標準製品仕様書（国土交通省）](#53942-３ｄ都市モデル標準製品仕様書国土交通省)
        - [5.3.9.4.3. Schema.org](#53943-schemaorg)
    - [5.3.10. 交番](#5310-交番)
      - [5.3.10.1. 参照元データ](#53101-参照元データ)
      - [5.3.10.2. データモデル](#53102-データモデル)
    - [5.3.11. 入浴施設](#5311-入浴施設)
      - [5.3.11.1. 参照元データ](#53111-参照元データ)
      - [5.3.11.2. データモデル](#53112-データモデル)
    - [5.3.12. クリーニング施設（コインランドリー含む）](#5312-クリーニング施設コインランドリー含む)
      - [5.3.12.1. 参照元データ](#53121-参照元データ)
      - [5.3.12.2. データモデル](#53122-データモデル)
  - [5.4. 　設備](#54-設備)
    - [5.4.1. 基本形](#541-基本形)
      - [5.4.1.1. 参照元データ](#5411-参照元データ)
      - [5.4.1.2. データモデル](#5412-データモデル)
      - [5.4.1.3. データ項目内で選択肢があるときの候補](#5413-データ項目内で選択肢があるときの候補)
        - [5.4.1.3.1. 区分](#54131-区分)
        - [5.4.1.3.2. 種類](#54132-種類)
      - [5.4.1.4. 既存データモデル](#5414-既存データモデル)
        - [5.4.1.4.1. ３Ｄ都市モデル標準製品仕様書](#54141-３ｄ都市モデル標準製品仕様書)
        - [5.4.1.4.2. Schema.org](#54142-schemaorg)
    - [5.4.2. 出入口](#542-出入口)
      - [5.4.2.1. 参照元データ](#5421-参照元データ)
      - [5.4.2.2. データモデル](#5422-データモデル)
      - [5.4.2.3. 既存データモデル](#5423-既存データモデル)
        - [5.4.2.3.1. GTFS](#54231-gtfs)
        - [5.4.2.3.2. ３Ｄ都市モデル標準製品仕様書](#54232-３ｄ都市モデル標準製品仕様書)
    - [5.4.3. AED](#543-aed)
      - [5.4.3.1. 参照元データ](#5431-参照元データ)
      - [5.4.3.2. データモデル](#5432-データモデル)
    - [5.4.4. 公衆無線LANアクセスポイント](#544-公衆無線lanアクセスポイント)
      - [5.4.4.1. 参照元データ](#5441-参照元データ)
      - [5.4.4.2. データモデル](#5442-データモデル)
    - [5.4.5. 公衆トイレ](#545-公衆トイレ)
      - [5.4.5.1. 参照元データ](#5451-参照元データ)
      - [5.4.5.2. データモデル](#5452-データモデル)
      - [5.4.5.3. 既存データモデル](#5453-既存データモデル)
        - [5.4.5.3.1. Schema.org](#54531-schemaorg)
    - [5.4.6. 消防水利施設](#546-消防水利施設)
      - [5.4.6.1. 参照元データ](#5461-参照元データ)
      - [5.4.6.2. データモデル](#5462-データモデル)
      - [5.4.6.3. データ項目に選択肢があるときの候補](#5463-データ項目に選択肢があるときの候補)
        - [5.4.6.3.1. オープンデータの推奨データセット](#54631-オープンデータの推奨データセット)
  - [5.5. 　　建物内・地下街・地下埋設物](#55-建物内地下街地下埋設物)
    - [5.5.1. 建物内・地下街](#551-建物内地下街)
    - [5.5.2. 地下埋設物](#552-地下埋設物)
    - [5.5.3. ボーリング柱状図等](#553-ボーリング柱状図等)
  - [5.6. 　道路](#56-道路)
    - [5.6.1. 道路](#561-道路)
      - [5.6.1.1. 参照元データ](#5611-参照元データ)
      - [5.6.1.2. データモデル](#5612-データモデル)
      - [5.6.1.3. データ項目に選択肢があるときの候補](#5613-データ項目に選択肢があるときの候補)
        - [5.6.1.3.1. 国土数値情報　緊急輸送道路　道路種別コード](#56131-国土数値情報緊急輸送道路道路種別コード)
      - [5.6.1.4. 既存データモデル](#5614-既存データモデル)
        - [5.6.1.4.1. ３Ｄ都市モデル標準製品仕様書](#56141-３ｄ都市モデル標準製品仕様書)
        - [5.6.1.4.2. 電子国土基本図　地図情報ファイル仕様書](#56142-電子国土基本図地図情報ファイル仕様書)
        - [5.6.1.4.3. Smart data model](#56143-smart-data-model)
    - [5.6.2. 道路関連設備](#562-道路関連設備)
    - [5.6.3. 交通規制情報](#563-交通規制情報)
    - [5.6.4. 交通インフラ（橋、トンネル等）](#564-交通インフラ橋トンネル等)
  - [5.7. 交通関連施設](#57-交通関連施設)
    - [5.7.1. 空港](#571-空港)
      - [5.7.1.1. 参照元データ](#5711-参照元データ)
      - [5.7.1.2. データモデル](#5712-データモデル)
      - [5.7.1.3. データ項目に選択肢があるときの候補](#5713-データ項目に選択肢があるときの候補)
      - [5.7.1.4. 既存データモデル](#5714-既存データモデル)
        - [5.7.1.4.1. 国土数値情報](#57141-国土数値情報)
        - [5.7.1.4.2. Smart Data Model](#57142-smart-data-model)
        - [5.7.1.4.3. Schema.org](#57143-schemaorg)
    - [5.7.2. 港湾](#572-港湾)
      - [5.7.2.1. 参照元データ](#5721-参照元データ)
      - [5.7.2.2. データモデル](#5722-データモデル)
      - [5.7.2.3. 既存データモデル](#5723-既存データモデル)
        - [5.7.2.3.1. 国土数値情報](#57231-国土数値情報)
        - [5.7.2.3.2. Smart data model](#57232-smart-data-model)
    - [5.7.3. 駅・バス停](#573-駅バス停)
      - [5.7.3.1. 参照元データ](#5731-参照元データ)
      - [5.7.3.2. データモデル](#5732-データモデル)
      - [5.7.3.3. 既存データモデル](#5733-既存データモデル)
        - [5.7.3.3.1. 国土数値情報](#57331-国土数値情報)
        - [5.7.3.3.2. GTFS[^38]](#57332-gtfs38)
        - [5.7.3.3.3. Smart data model](#57333-smart-data-model)
        - [5.7.3.3.4. Schema.org](#57334-schemaorg)
    - [5.7.4. 鉄道路線・バス路線](#574-鉄道路線バス路線)
      - [5.7.4.1. 参照元データ](#5741-参照元データ)
      - [5.7.4.2. データモデル](#5742-データモデル)
      - [5.7.4.3. データ項目に選択肢があるときの候補](#5743-データ項目に選択肢があるときの候補)
        - [5.7.4.3.1. GTFSの経路タイプ](#57431-gtfsの経路タイプ)
      - [5.7.4.4. 既存データモデル](#5744-既存データモデル)
        - [5.7.4.4.1. 国土数値情報](#57441-国土数値情報)
        - [5.7.4.4.2. GTFS](#57442-gtfs)
    - [5.7.5. タクシー乗り場](#575-タクシー乗り場)
      - [5.7.5.1. 参照元データ](#5751-参照元データ)
      - [5.7.5.2. データモデル](#5752-データモデル)
      - [5.7.5.3. 既存データモデル](#5753-既存データモデル)
        - [5.7.5.3.1. Schema.org](#57531-schemaorg)
    - [5.7.6. 駐車場](#576-駐車場)
      - [5.7.6.1. 参照元データ](#5761-参照元データ)
      - [5.7.6.2. データモデル](#5762-データモデル)
      - [5.7.6.3. 既存データモデル](#5763-既存データモデル)
        - [5.7.6.3.1. Smart data model](#57631-smart-data-model)
        - [5.7.6.3.2. Schema.org](#57632-schemaorg)
    - [5.7.7. 駐車スペース（路上）](#577-駐車スペース路上)
      - [5.7.7.1. 参照元データ](#5771-参照元データ)
      - [5.7.7.2. データモデル](#5772-データモデル)
      - [5.7.7.3. 既存データモデル](#5773-既存データモデル)
        - [5.7.7.3.1. Smart data model](#57731-smart-data-model)
    - [5.7.8. 駐車スポット（荷さばき場所）](#578-駐車スポット荷さばき場所)
      - [5.7.8.1. 参照元データ](#5781-参照元データ)
      - [5.7.8.2. データモデル](#5782-データモデル)
      - [5.7.8.3. 既存データモデル](#5783-既存データモデル)
        - [5.7.8.3.1. Smart data model](#57831-smart-data-model)
    - [5.7.9. 駐輪場](#579-駐輪場)
      - [5.7.9.1. 参照元データ](#5791-参照元データ)
      - [5.7.9.2. データモデル](#5792-データモデル)
    - [5.7.10. シェアードカーステーション](#5710-シェアードカーステーション)
    - [5.7.11. レンタル自転車・ｅスクータースポット](#5711-レンタル自転車ｅスクータースポット)
      - [5.7.11.1. 参照元データ](#57111-参照元データ)
      - [5.7.11.2. データモデル](#57112-データモデル)
      - [5.7.11.3. 既存データモデル](#57113-既存データモデル)
        - [5.7.11.3.1. Smart data model](#571131-smart-data-model)
        - [5.7.11.3.2. GBFS（General Bikeshare Feed Specification）[^39]](#571132-gbfsgeneral-bikeshare-feed-specification39)
    - [5.7.12. ガソリンスタンド・水素ステーション](#5712-ガソリンスタンド水素ステーション)
      - [5.7.12.1. 参照元データ](#57121-参照元データ)
      - [5.7.12.2. データモデル](#57122-データモデル)
      - [5.7.12.3. 既存データモデル](#57123-既存データモデル)
        - [5.7.12.3.1. Schema.org](#571231-schemaorg)
        - [5.7.12.3.2. Smart data model](#571232-smart-data-model)
    - [5.7.13. EVスタンド](#5713-evスタンド)
      - [5.7.13.1. 参照元データ](#57131-参照元データ)
      - [5.7.13.2. データモデル](#57132-データモデル)
      - [5.7.13.3. 既存データモデル](#57133-既存データモデル)
        - [5.7.13.3.1. Smart data model](#571331-smart-data-model)
  - [5.8. 　地物・地点](#58-地物地点)
      - [5.8.0.1. 参照元データ](#5801-参照元データ)
      - [5.8.0.2. データモデル](#5802-データモデル)
      - [5.8.0.3. データ項目に選択肢があるときの候補](#5803-データ項目に選択肢があるときの候補)
      - [5.8.0.4. 既存データモデル](#5804-既存データモデル)
        - [5.8.0.4.1. Smart Data Model](#58041-smart-data-model)
        - [5.8.0.4.2. Schema.org](#58042-schemaorg)
    - [5.8.1. 観光ポイント](#581-観光ポイント)
      - [5.8.1.1. 参照元データ](#5811-参照元データ)
      - [5.8.1.2. データモデル](#5812-データモデル)
      - [5.8.1.3. データ項目に選択肢があるときの候補](#5813-データ項目に選択肢があるときの候補)
      - [5.8.1.4. 既存データモデル](#5814-既存データモデル)
        - [5.8.1.4.1. 国土数値情報](#58141-国土数値情報)
        - [5.8.1.4.2. Smart Data Model](#58142-smart-data-model)
        - [5.8.1.4.3. Schema.org](#58143-schemaorg)
    - [5.8.2. 指定緊急避難場所](#582-指定緊急避難場所)
      - [5.8.2.1. 参照元データ](#5821-参照元データ)
      - [5.8.2.2. データモデル](#5822-データモデル)
    - [5.8.3. ゴミ集積場所](#583-ゴミ集積場所)
      - [5.8.3.1. 参照元データ](#5831-参照元データ)
      - [5.8.3.2. データモデル](#5832-データモデル)
    - [5.8.4. その他地点](#584-その他地点)
  - [5.9. 　移動オブジェクト](#59-移動オブジェクト)
    - [5.9.1. 基本形](#591-基本形)
      - [5.9.1.1. 参照元データ](#5911-参照元データ)
      - [5.9.1.2. データモデル](#5912-データモデル)
      - [5.9.1.3. データ項目に選択肢があるときの候補](#5913-データ項目に選択肢があるときの候補)
    - [5.9.2. 人](#592-人)
      - [5.9.2.1. 参照元データ](#5921-参照元データ)
      - [5.9.2.2. データモデル](#5922-データモデル)
      - [5.9.2.3. データ項目に選択肢があるときの候補](#5923-データ項目に選択肢があるときの候補)
        - [5.9.2.3.1. 性別](#59231-性別)
        - [5.9.2.3.2. 移動、視覚、聴覚条件](#59232-移動視覚聴覚条件)
      - [5.9.2.4. 既存データモデル](#5924-既存データモデル)
        - [5.9.2.4.1. 人の流れ（パーソントリップ）に関するデータモデル](#59241-人の流れパーソントリップに関するデータモデル)
    - [5.9.3. 乗り物（自動車、オートバイ、自転車、船、飛行物、その他）](#593-乗り物自動車オートバイ自転車船飛行物その他)
      - [5.9.3.1. 参照元データ](#5931-参照元データ)
      - [5.9.3.2. データモデル](#5932-データモデル)
      - [5.9.3.3. データ項目に選択肢があるときの候補](#5933-データ項目に選択肢があるときの候補)
        - [5.9.3.3.1. 乗り物タイプ](#59331-乗り物タイプ)
        - [5.9.3.3.2. 燃料種別](#59332-燃料種別)
    - [5.9.4. 物](#594-物)
  - [5.10. 　自然](#510-自然)
    - [5.10.1. 植生](#5101-植生)
    - [5.10.2. 湖沼](#5102-湖沼)
    - [5.10.3. 河川](#5103-河川)
    - [5.10.4. 自然の保護保全](#5104-自然の保護保全)
    - [5.10.5. 天気](#5105-天気)
      - [5.10.5.1. データ項目に選択肢があるときの候補](#51051-データ項目に選択肢があるときの候補)
  - [5.11. 　農地](#511-農地)
      - [5.11.0.1. 参照元データ](#51101-参照元データ)
      - [5.11.0.2. データモデル](#51102-データモデル)
      - [5.11.0.3. データ項目に選択肢があるときの候補](#51103-データ項目に選択肢があるときの候補)
        - [5.11.0.3.1. 地目](#511031-地目)
      - [5.11.0.4. 既存データモデル](#51104-既存データモデル)
        - [5.11.0.4.1. Smart data model](#511041-smart-data-model)
  - [5.12. 　イベント・アクション](#512-イベントアクション)
    - [5.12.1. イベント](#5121-イベント)
      - [5.12.1.1. 参照元データ](#51211-参照元データ)
      - [5.12.1.2. データモデル](#51212-データモデル)
      - [5.12.1.3. データ項目に選択肢があるときの候補](#51213-データ項目に選択肢があるときの候補)
        - [5.12.1.3.1. イベント種類](#512131-イベント種類)
        - [5.12.1.3.2. 体験種類](#512132-体験種類)
      - [5.12.1.4. 既存データモデル](#51214-既存データモデル)
        - [5.12.1.4.1. Smart data model](#512141-smart-data-model)
        - [5.12.1.4.2. Schema.org](#512142-schemaorg)
    - [5.12.2. アクション](#5122-アクション)
      - [5.12.2.1. 参照元データ](#51221-参照元データ)
      - [5.12.2.2. データモデル](#51222-データモデル)
  - [5.13. 　緊急情報・防災情報](#513-緊急情報防災情報)
  - [5.14. 　行政情報](#514-行政情報)
    - [5.14.1. 行政情報](#5141-行政情報)
      - [5.14.1.1. 参照元データ](#51411-参照元データ)
      - [5.14.1.2. データモデル](#51412-データモデル)
      - [5.14.1.3. 既存データモデル](#51413-既存データモデル)
        - [5.14.1.3.1. 統計](#514131-統計)
    - [5.14.2. 地域・年齢別人口](#5142-地域年齢別人口)
      - [5.14.2.1. 参照元データ](#51421-参照元データ)
      - [5.14.2.2. データモデル](#51422-データモデル)
  - [5.15. 　センサーデータ](#515-センサーデータ)
      - [5.15.0.1. 参照元データ](#51501-参照元データ)
      - [5.15.0.2. データモデル](#51502-データモデル)
      - [5.15.0.3. データ項目に選択肢があるときの候補](#51503-データ項目に選択肢があるときの候補)
        - [5.15.0.3.1. 種類](#515031-種類)
      - [5.15.0.4. 既存データモデル](#51504-既存データモデル)
        - [5.15.0.4.1. Smart data model](#515041-smart-data-model)
        - [5.15.0.4.2. OMA（Open Mobile Alliance）](#515042-omaopen-mobile-alliance)
        - [5.15.0.4.3. GSM Association](#515043-gsm-association)
        - [5.15.0.4.4. W3C SSN(Semantic Sensor Network Ontology)](#515044-w3c-ssnsemantic-sensor-network-ontology)
  - [5.16. 　健康情報](#516-健康情報)
    - [5.16.1. 健康情報](#5161-健康情報)
    - [5.16.2. 給食情報](#5162-給食情報)
      - [5.16.2.1. 参照元データ](#51621-参照元データ)
      - [5.16.2.2. データモデル](#51622-データモデル)
      - [5.16.2.3. データ項目に選択肢があるときの候補](#51623-データ項目に選択肢があるときの候補)
  - [5.17. 　文化財](#517-文化財)
    - [5.17.1. 建物、施設、もの、地物、地域の文化財](#5171-建物施設もの地物地域の文化財)
      - [5.17.1.1. 参照元データ](#51711-参照元データ)
      - [5.17.1.2. データモデル](#51712-データモデル)
      - [5.17.1.3. データ項目に選択肢があるときの候補](#51713-データ項目に選択肢があるときの候補)
        - [5.17.1.3.1. 分類](#517131-分類)
        - [5.17.1.3.2. 種類](#517132-種類)
      - [5.17.1.4. 既存データモデル](#51714-既存データモデル)
        - [5.17.1.4.1. 国指定文化財等データベース](#517141-国指定文化財等データベース)
    - [5.17.2. 無形の文化財](#5172-無形の文化財)
      - [5.17.2.1. 参照元データ](#51721-参照元データ)
      - [5.17.2.2. データモデル](#51722-データモデル)
- [6. 　コード体系、ID体系](#6-コード体系id体系)
  - [6.1. 　地理空間に関するコード](#61-地理空間に関するコード)
    - [6.1.1. 地方公共団体コード](#611-地方公共団体コード)
    - [6.1.2. POIコード](#612-poiコード)
  - [6.2. 　時間、時期に関するコード](#62-時間時期に関するコード)
    - [6.2.1. 時間帯コード](#621-時間帯コード)
    - [6.2.2. 季節、旬コード](#622-季節旬コード)
  - [6.3. 　状態に関するコード](#63-状態に関するコード)
    - [6.3.1. 施設やイベントの状況](#631-施設やイベントの状況)
    - [6.3.2. 混雑の状況](#632-混雑の状況)
    - [6.3.3. 予約の状況](#633-予約の状況)
  - [6.4. ユニークID（識別子）](#64-ユニークid識別子)
    - [6.4.1. 行政機関が発行する主なユニークID](#641-行政機関が発行する主なユニークid)
    - [6.4.2. 行政機関に要望が多いユニークID](#642-行政機関に要望が多いユニークid)
- [7. 　成果測定指標（KGI,KPI）](#7-成果測定指標kgikpi)
  - [7.1. 　スマートシティ施策のKPI設定指針](#71-スマートシティ施策のkpi設定指針)
  - [7.2. 　デジタル田園都市　wellbeing指標](#72-デジタル田園都市wellbeing指標)
  - [7.3. 　都市再生のための評価マニュアル　モニタリングマニュアル](#73-都市再生のための評価マニュアルモニタリングマニュアル)
- [8. 　データカタログ等](#8-データカタログ等)
  - [8.1. 　データカタログ](#81-データカタログ)
  - [8.2. 　データセットのメタデータ](#82-データセットのメタデータ)
    - [8.2.1. カタログ](#821-カタログ)
    - [8.2.2. カタログレコード](#822-カタログレコード)
    - [8.2.3. データセット](#823-データセット)
    - [8.2.4. データサービス](#824-データサービス)
    - [8.2.5. ディストリビューション](#825-ディストリビューション)
  - [8.3. 　データディクショナリ](#83-データディクショナリ)
- [9. 　データの運用](#9-データの運用)
  - [9.1. 　データのエコシステムの構築](#91-データのエコシステムの構築)
  - [9.2. 　データの格納形態、送信形態](#92-データの格納形態送信形態)
  - [9.3. 　データ品質](#93-データ品質)
- [10. 　Smart Data Model、３D都市モデル、schema.orgとのマッピング](#10-smart-data-model３d都市モデルschemaorgとのマッピング)
  - [10.1. 　土地](#101-土地)
  - [10.2. 　建物](#102-建物)
  - [10.3. 　施設](#103-施設)
  - [10.4. 　設備](#104-設備)
...
###### 5.1.1.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）
...
###### 5.2.0.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）
...
###### 5.3.1.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）
...
###### 5.3.2.4.2. ３Ｄ都市モデル標準製品仕様書（国土交通省）

urf:UrbanFacility、urf:DistrictFacilityの定義および派生形が参照できます。

###### 5.3.2.4.3. Smart Data Model

スマートシティ用のBuildingデータモデルの建物の基本情報に加え、BuildingOperationやBuildhingTypeのデータ項目を組み合わせて使用するモデルです。

###### 5.3.2.4.4. Schema.org
...
###### 5.3.3.4.1. 厚生労働省各地方局の「保険医療機関・保険薬局の指定等一覧」

厚生労働省各地方局が毎月公表する「保険医療機関・保険薬局の指定等一覧」が参照できます。

###### 5.3.3.4.2. 国土数値情報の「医療機関」

国土数値情報の医療情報[^31]が参照できます。

###### 5.3.3.4.3. ３Ｄ都市モデル標準製品仕様書（国土交通省）

urf:MedicalFacilityのデータ項目が参照できます。

###### 5.3.3.4.4. Schema.org
...
###### 5.3.5.4.1. ３Ｄ都市モデル標準製品仕様書（国土交通省）

urf:SocialWelfareFacilityのデータ項目が参照できます。

###### 5.3.5.4.2. Schema.org
...
ｇBizInfo[^62]のデータモデルに、目的別追加データ項目として以下を付加して利用します。
...
##### 5.3.8.3. データ項目に選択肢があるときの候補

###### 5.3.8.3.1. 食品衛生法施行令第35条
...
###### 5.4.1.4.1. ３Ｄ都市モデル標準製品仕様書

frn:CityFurnitureのデータモデルが参照できます。

###### 5.4.1.4.2. Schema.org
...
##### 5.4.6.3. データ項目に選択肢があるときの候補

###### 5.4.6.3.1. オープンデータの推奨データセット
...
##### 5.6.1.3. データ項目に選択肢があるときの候補

###### 5.6.1.3.1. 国土数値情報　緊急輸送道路　道路種別コード
...
###### 5.6.1.4.1. ３Ｄ都市モデル標準製品仕様書

Tran:roadのモデルが参照できます。車道、歩道など詳細な３Dモデルを作成できます。

###### 5.6.1.4.2. 電子国土基本図　地図情報ファイル仕様書

交通施設のサブパッケージのデータモデルを参照できます。

###### 5.6.1.4.3. Smart data model
...
###### 5.7.3.3.3. Smart data model

TransportStation、PublicTransportStopが参照できます。

###### 5.7.3.3.4. Schema.org
...
##### 5.7.4.3. データ項目に選択肢があるときの候補

###### 5.7.4.3.1. GTFSの経路タイプ
...
#### 5.7.10. シェアードカーステーション

シェアードカーステーションは、次項目のレンタル自転車のモデルを準用します。

#### 5.7.11. レンタル自転車・ｅスクータースポット
...
###### 5.7.11.3.1. Smart data model

BikeHireDockingStationでサービス会社やサービスエリアのデータ項目を定義しています。

###### 5.7.11.3.2. GBFS（General Bikeshare Feed Specification）[^39]

バイクシェアなどのリアルタイムデータを配信する仕様として整備されたjsonファイルの集合です。

#### 5.7.12. ガソリンスタンド・水素ステーション
...
##### 5.8.0.3. データ項目に選択肢があるときの候補

* POIは、GIFコード（POIコード）を参照します。

##### 5.8.0.4. 既存データモデル

###### 5.8.0.4.1. Smart Data Model

Point of Interestのデータ項目を参照できます。

###### 5.8.0.4.2. Schema.org
...
###### 5.9.2.4.1. 人の流れ（パーソントリップ）に関するデータモデル

「東京大学CSIS人の流れデータセット」[^43]や「交通流動量 パーソントリップＯＤ量データ」[^44]があります。

#### 5.9.3. 乗り物（自動車、オートバイ、自転車、船、飛行物、その他）
...
##### 5.9.3.3. データ項目に選択肢があるときの候補

###### 5.9.3.3.1. 乗り物タイプ

乗り物タイプは以下から選択します。

乗用車、トラック、バス、農耕車両、工事車両、オートバイ、自転車、車いす、動力付きスクーター、UAV
...
###### 5.12.1.4.1. Smart data model

Eventのデータを定義しています。出演者などの詳細情報のデータ項目が参照できます。

###### 5.12.1.4.2. Schema.org
...
申請・届出、証明・通知、事例、行政サービス、イベント、報告書、行政サービス拠点・支援機関、調達
...
##### 5.15.0.3. データ項目に選択肢があるときの候補

###### 5.15.0.3.1. 種類

センサーの種類は、以下から選択できます。

大気、騒音、温度、湿度、風速、風向、圧力、水位、水質、化学物質濃度、酸性度、照度、振動、加速度、ひずみ、人流、交通量、その他

##### 5.15.0.4. 既存データモデル

###### 5.15.0.4.1. Smart data model

Deviceは、センサーとアクチュエータを含んだデータモデルです。誤差や製造番号等の詳細データ項目が定義されています。

###### 5.15.0.4.2. OMA（Open Mobile Alliance）

地域に設置されるセンサーなどモバイルに関連したセンサーのデータモデルが定義されています。[^48]

###### 5.15.0.4.3. GSM Association

地域に設置されるセンサーなどモバイルに関連したセンサーのデータモデルが定義されています。[^49]

###### 5.15.0.4.4. W3C SSN(Semantic Sensor Network Ontology)
...
「受付開始前」「予約可」、「要問合せ」、「予約不可」、「当日可」、「キャンセル待可」、「予約不要」、「予約期間終了」、「先着順」
...
### 7.3. 　都市再生のための評価マニュアル　モニタリングマニュアル
...
<figure>
<img src="md/452-1/media/image8.jpg" style="width:5.73542in;height:2.47361in" />
...
---
## 10. 　Smart Data Model、３D都市モデル、schema.orgとのマッピング
